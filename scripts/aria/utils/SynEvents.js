...
(function () {
    var _isInDom = function (element) {
        if (!element || element == Aria.$window || element == Aria.$window.document || !element.parentNode) {
            return false;
        }

        try {
            while (element != Aria.$window.document) {
    ...
    var _isWithin = function (needle, haystack) {
        var parent = needle;
        try {
            if (parent && parent !== Aria.$window.document && !parent.parentNode) {
                return;
            }

            while (parent && parent !== haystack) {
    ...
    var _fireMouseEnterLeave = function (last, el, options) {
        // The events mouseenter/leave don't bubble, however when we move from last to el
        // we might cross more that one element, thus entering/leaving in multiple boxes
        var parent;
        if (_isWithin(last, el)) {
            // last is inside el, means that we are departing from last
            Aria.$global.Syn.trigger("mouseleave", options, last);
            parent = last.parentNode;
            if (parent && el !== parent && parent != Aria.$window.document) {
                _fireMouseEnterLeave(parent, el, options);
            }
        } else {
            // last is not inside el, means that we are entering inside el
            Aria.$global.Syn.trigger("mouseenter", options, el);
            parent = el.parentNode;
            if (parent && last !== parent && parent != Aria.$window.document) {
                _fireMouseEnterLeave(last, parent, options);
            }
        }
    };
     ...
     * Changes with master
     *
     * - Inside synthetic.js focus method, blur only if active element is not the body
    ...
    /*!
     * https://raw.github.com/bitovi/syn/master/synthetic.js 2013.05.14
     * Copyright (C) 2012 by Bitovi under MIT license
     */
    steal(function(){
        ...
        extend(Syn, {
            ...
            jquery: function( el, fast ) {
                if ( Aria.$window.FuncUnit && Aria.$window.FuncUnit.jQuery ) {
                    return Aria.$window.FuncUnit.jQuery;
                }
                if ( el ) {
                    return Syn.helpers.getWindow(el).jQuery || Aria.$window.jQuery;
                }
                else {
                    return Aria.$window.jQuery;
                }
            },
            /**
            ...
            args: function() {
                ...
                for ( ; i < arguments.length; i++ ) {
                    if ( typeof arguments[i] === 'function' ) {
                        ...
                        res.element = Aria.$window.document.getElementById(arguments[i]);
                    }
                    else if ( arguments[i] ) {
            ...
            helpers: {
                ...
                scrollOffset: function( win , set) {
                    ...
                    if(set){
                        Aria.$window.scrollTo(set.left, set.top);
            ...
            create: {
                ...
                page: {
                    event: function( type, options, element ) {
                        var doc = Syn.helpers.getWindow(element).document || Aria.$window.document,
                            event;
                        if ( doc.createEvent ) {
                            event = doc.createEvent("Events");
                        ...
                        Syn.onParents(element, function( el ) {
                            if ( Syn.isFocusable(el) ) {
                                if ( el.nodeName.toLowerCase() !== 'html' ) {
                                    ...
                                    // TODO: The HTML element isn't focusable in IE, but it is
                                    // in FF.  We should detect this and do a true focus instead
                                    // of just a blur
                                    var doc = Syn.helpers.getWindow(element).document;
                                    if ( doc !== Aria.$window.document ) {
                                        return false;
                                    } else if ( doc.activeElement ) {
                                        if (doc.activeElement != Aria.$window.document.body) {
            ...
            eventSupported: function( eventName ) {
                var el = Aria.$window.document.createElement("div");
    ...
    /*!
     * https://raw.github.com/bitovi/syn/master/mouse.js 2013.05.14
     * Copyright (C) 2012 by Bitovi under MIT license
     */
    steal('./synthetic',function(Syn) {
        ...
        h.extend(Syn.defaults, {
            ...
            click: function() {
                ...
                var element = this, href, type, radioChanged, createChange, nodeName, scope;
                try {
                    href = element.href;
                    type = element.type;
        ...
        h.extend(Syn.create, {
            mouse: {
                options: function( type, options, element ) {
                    var doc = Aria.$window.document.documentElement,
                        body = Aria.$window.document.body,
                    ...
                    return h.extend({
                        ...
                        relatedTarget: Aria.$window.document.documentElement
                    }, options);
                },
                event: function( type, defaults, element ) { //Everyone Else
                    var doc = getWin(element).document || Aria.$window.document;
                    if ( doc.createEvent ) {
                        var event;
    ...
    /*!
     * https://raw.github.com/bitovi/syn/master/browsers.js 2013.05.14
     * Copyright (C) 2012 by Bitovi under MIT license
     */
    steal('./synthetic','./mouse.js',function(Syn) {
        ...
        Syn.key.browser =
        (function(){
            if(Syn.key.browsers[Aria.$global.navigator.userAgent]){
                return Syn.key.browsers[Aria.$global.navigator.userAgent];
            }
        ...
        Syn.mouse.browser =
        (function(){
            if(Syn.mouse.browsers[Aria.$global.navigator.userAgent]){
                return Syn.mouse.browsers[Aria.$global.navigator.userAgent];
            }
            for(var browser in Syn.browser){
    ...
    /*!
     * https://raw.github.com/bitovi/syn/master/key.js 2013.05.14
     * Copyright (C) 2012 by Bitovi under MIT license
     */
        ...
        h.extend(Syn.create, {
            ...
            key: {
                ...
                event: function( type, options, element ) { //Everyone Else
                    var doc = h.getWindow(element).document || Aria.$window.document;
                    if ( doc.createEvent ) {
                        var event;
                        ...
                        try {
                            ...
                            event.initKeyEvent(type, true, true, Aria.$window, options.ctrlKey, options.altKey, options.shiftKey, options.metaKey, options.keyCode, options.charCode);
                        }
                        catch (e) {
    ...
    /*!
     * https://raw.github.com/bitovi/syn/master/drag/drag.js 2013.05.14
     * Copyright (C) 2012 by Bitovi under MIT license
     */
    steal('../synthetic.js',function(Syn) {
        ...
        (function() {
            ...
            Aria.$window.document.body.scrollTop = 11;
            if (!Aria.$window.document.elementFromPoint ) {
                return;
            }
            var el = Aria.$window.document.elementFromPoint(3, 1);
            if ( el == div ) {
            ...
            Aria.$window.document.body.removeChild(div);
        ...
        var elementFromPoint = function( point, element ) {
            ...
            mouseMove = function( point, element, last ) {
                ...
                if ( last != el && el && last ) {
                    ...
                    if (Syn.browser.msie) {
                        _fireMouseEnterLeave(last, el, options);
                    }
                }
            ...
            center = function( el ) {
                var docElem = (el && el.ownerDocument).documentElement;
                var box = {
                    top : 0, left : 0
                };
